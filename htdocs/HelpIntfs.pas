...
begin
  ...
  try
    ...
    begin
      ...
      Viewer := THelpViewerNode(ViewerNames.Objects[0]).Viewer;
    end
    else if (ViewerNames.Count > 0) and (FHelpSelector <> nil) then
    begin
      // If there is more than one TOC provider, let the user choose the one to
      // use.
      ViewerNames.Sort;
      I := FHelpSelector.TableOfContents(ViewerNames);
      Viewer := THelpViewerNode(ViewerNames.Objects[I]).Viewer;
    end;
  finally
    ViewerNames.Free;
  end;
